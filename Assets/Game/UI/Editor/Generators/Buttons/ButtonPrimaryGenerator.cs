...
namespace Game.UI.Editor.Generators.Buttons
{
    ...
    /// 主按钮生成器
    /// </summary>
    public static class ButtonPrimaryGenerator
    {
        private const string Dir = "Assets/Game/UI/Common/Buttons";
        private const string FileName = "UI_ButtonPrimary.prefab";

        [MenuItem("Tools/Game UI/Generators/Buttons/生成主按钮")]
        public static void Generate()
        {
            UIGeneratorUtility.EnsureFolder(Dir);

            var path = $"{Dir}/{FileName}";
            var root = new GameObject("UI_ButtonPrimary", typeof(RectTransform), typeof(Image), typeof(Button));
            var rect = root.GetComponent<RectTransform>();
            rect.sizeDelta = new Vector2(320f, 88f);

            var image = root.GetComponent<Image>();
            image.color = UIGeneratorUtility.ColorPrimary;

            var button = root.GetComponent<Button>();
            button.transition = Selectable.Transition.ColorTint;
            button.targetGraphic = image;
            button.colors = UIGeneratorUtility.BuildButtonColors(isPrimary: true);
            ...
            UIGeneratorUtility.MarkAddressable(path, UIKeys.Common.ButtonPrimary);

            AssetDatabase.SaveAssets();
            Debug.Log($"[UI生成器] ✅ 已生成：{UIKeys.Common.ButtonPrimary}");
        }
    }
}
